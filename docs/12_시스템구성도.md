...
이것은 구성의 문제가 아니라 **이행이 안 끝난 것**이다. 근거와 선택지는
`코드검토_2026-08-24_팀원머지.md` §2 에 있고, 결정은 14 §7 미결에 있다.
**이 문서는 결정된 뒤의 구성을 그린다** — 위 표의 "소유: Django" 는 목표 상태다.
...
> **옛** 14 §3.4 초안이 *"8000(추론)은 보안그룹에서 차단"* 이라고 적고 있었다.
...
- [ ] `TIME_ZONE = "UTC"` 인데 `LANGUAGE_CODE = "ko-kr"` 다 (`settings.py:148`).
      다이어리의 `recorded_date` 가 **하루 어긋날 수 있다** — 오전 8시 KST = 전날 23시 UTC.
      확인 필요
